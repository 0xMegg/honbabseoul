...
- `src/setup.sh:158-186` — `FILES_TO_REPLACE` 에 `.claude/commands/task.md` 명시적 추가 + `` 자동 발견 보조 루프 (`.claude/commands/`, `.claude/scripts/` 재귀 grep).
- `src/scripts/upgrade-harness.sh`:
  ...
  - managed 분기 post-copy 치환 (managed `*.md`/`*.sh` 에 `` 매칭 시 sed)
...
- Fix 2: divebase 의 `task.md` 에 `` 가 그대로 남음. setup.sh `FILES_TO_REPLACE` 누락 + upgrade-harness.sh 가 `[managed]` 파일을 raw 복사하기만 한 게 원인.
...
- 모든 다운스트림 (divebase, honbabseoul, kody, …): 다음 sync 시 `/task` monitoring prompt 폭주 회복. task.md 의 `` 자동 치환. base/local split 이후 옛 root rules 보유한 프로젝트는 5개 자동 삭제.
...
2. `bash scripts/upgrade-harness.sh --apply` 후 `grep -r '' .claude/` → match 0.
